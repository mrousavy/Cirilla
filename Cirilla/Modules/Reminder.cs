        ...
        public async Task RemindMe(
            ...
            try {
                ...
                await ConsoleHelper.Log(
                    $"{Helper.GetName(Context.User)} set a reminder for {(DateTime.Now + time):dd.MM.yyyy HH:mm}!",
                    LogSeverity.Info);
                await ReplyAsync(
                    $"All set {Helper.GetName(Context.User)}, I'll remind you at {(DateTime.Now + time):dd.MM.yyyy HH:mm}!");
